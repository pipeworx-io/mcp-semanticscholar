...
/**
 * LICENCE (fleet #1974). The key we hold was issued under the Semantic Scholar
 * API License Agreement (https://api.semanticscholar.org/license/). Section
 * 2(a) limits use to "legitimate, non-commercial, research and/or educational
 * purposes", requires that "any public use of Data must point back to Semantic
 * Scholar at https://www.semanticscholar.org/ with a utm_source=api UTM
 * parameter", and the Use Restrictions forbid "sell, lease, share, transfer,
 * sublicense, commercialize ... any Data obtained through the API". So:
 *   - `zeroRated: true` on the gateway pack entry: 0 credits on every tier;
 *   - this envelope leads every response so the point-back and the
 *     non-commercial condition reach the caller, who is bound downstream too.
 * Zero-rating removes the fee; it does not license onward sharing — the
 * caller's own use has to be non-commercial as well, and the note says so.
 */
const S2_LICENSE = vendorTermsLicense({
  ref: 'SemanticScholar-API-License-Agreement',
  url: 'https://api.semanticscholar.org/license/',
  attribution: 'Data from Semantic Scholar, Allen Institute for AI — https://www.semanticscholar.org/?utm_source=api',
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Semantic Scholar Academic Graph');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  return attachLicense(await dispatch(name, args), S2_LICENSE);
}
...
async function searchPapers(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(url, { headers: s2Headers(args) });
  if (res.status === 429) return rateLimited();
  if (!res.ok) return { error: `Semantic Scholar: ${res.status} ${(await res.text()).slice(0, 200)}` };

  const body = (await parseJson<unknown>(res, 'Semantic Scholar')) as Record<string, unknown>;
...
async function getPaper(args: Record<string, unknown>): Promise<unknown> {
  ...
  const p = (await parseJson<unknown>(res, 'Semantic Scholar')) as Record<string, unknown>;
  const externalIds = (p.externalIds as Record<string, unknown> | undefined) ?? undefined;
  const openAccessPdf = (p.openAccessPdf as Record<string, unknown> | undefined)?.url;
